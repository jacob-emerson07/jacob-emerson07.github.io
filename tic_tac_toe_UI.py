...
def take_input(board):
    ...
    while not is_valid:
        ...
        if ttt.validate_turn(board, row, column):
            is_valid = True
            break
...
def start():
    ...
    while RUNNING:
        ...
        if game_type == "Easy" and ttt.TURN == 2:
            print()
            coords = ttt_AI.easy_input(board)

        elif game_type == "Medium" and ttt.TURN == 2:
            print()
            coords = ttt_AI.medium_input(board)
            
        else:
            coords = take_input(board)

        board = ttt.add_piece(board, coords)

        victor = ttt.winner(board)
